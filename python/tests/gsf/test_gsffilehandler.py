...
from themachinethatgoesping.echosounders import gsf


LOGGER = logging.getLogger(__name__)


class Test_echosounders_gsf_GSFFileHandler:
    files = None
    ...
    def open_files_and_loop_through_pings(self, file, cache=False):
        if cache:
            fm = gsf.GSFFileHandler(file, index_paths=self.index_files, show_progress=False)
        else:
            fm = gsf.GSFFileHandler(file, show_progress=False)
