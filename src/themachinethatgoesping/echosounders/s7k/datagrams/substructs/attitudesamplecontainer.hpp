...
/**
 * @brief Container for the attitude attitudes of a 1016 Attitude record.
 *
 * The attitudes are stored as one contiguous block and read as a single bulk read. Per-field numpy
...
class AttitudeSampleContainer
{
  public:
    ...
    // ----- raw struct access -----
    const std::vector<AttitudeSample>& get_attitudes() const;
    std::vector<AttitudeSample>&       attitudes();
    void                               set_attitudes(const std::vector<AttitudeSample>& attitudes);
    ...
    // ----- processed -----
    xt::xtensor<double, 1> get_delta_time_in_seconds_tensor() const;
    xt::xtensor<float, 1>  get_roll_in_degrees_tensor() const;
    xt::xtensor<float, 1>  get_pitch_in_degrees_tensor() const;
    xt::xtensor<float, 1>  get_heading_in_degrees_tensor() const;
  ...
  private:
    std::vector<AttitudeSample> _attitudes;
