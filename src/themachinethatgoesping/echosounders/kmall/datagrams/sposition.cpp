...
// ----- setters -----
void SPosition::set_pos_data_from_sensor(std::string_view pos_data)
{
...
void SPosition::__read__(std::istream& is)
{
    static constexpr size_t dbytes = __size + KMALLSensorDatagram::__size + sizeof(uint32_t);
...
SPosition SPosition::from_stream(std::istream& is, const KMALLDatagram& header)
{
    SPosition datagram(header);
...
SPosition SPosition::from_stream(std::istream& is, o_KMALLDatagramIdentifier datagram_identifier)
{
    SPosition datagram;
...
SPosition SPosition::from_stream(std::istream& is)
{
    SPosition datagram;
    datagram.__kmalldatagram_read__(is);
    datagram.__read_sensordatagram__(is);
...
void SPosition::to_stream(std::ostream& os) const
{
...
tools::classhelper::ObjectPrinter SPosition::__printer__(unsigned int float_precision,
