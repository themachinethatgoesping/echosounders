...
#include ".docstrings/mwctxsectorscontainer.doc.hpp"
...
/**
 * @brief Container that stores a vector of MWCSectorInfo and exposes per-field
 *        vectorised (xtensor) accessors for fast bulk access.
 */
class MWCTxSectorsContainer
{
  public:
    MWCTxSectorsContainer()  = default;
    ~MWCTxSectorsContainer() = default;

    // ----- vector access -----
    const std::vector<MWCSectorInfo>& get_tx_sectors() const;
    std::vector<MWCSectorInfo>&       tx_sectors();
    void set_tx_sectors(const std::vector<MWCSectorInfo>& sectors);
    ...
    xt::xtensor<float, 1> get_centre_frequency_hz_tensor(
        const std::vector<uint32_t>& indices = {}) const;
    xt::xtensor<float, 1> get_tx_beam_width_along_deg_tensor(
        const std::vector<uint32_t>& indices = {}) const;
    xt::xtensor<uint16_t, 1> get_tx_sector_number_tensor(
        const std::vector<uint32_t>& indices = {}) const;
    xt::xtensor<int16_t, 1> get_padding_tensor(
        const std::vector<uint32_t>& indices = {}) const;
    ...
    // ----- operators -----
    bool operator==(const MWCTxSectorsContainer& other) const = default;
  ...
  private:
    std::vector<MWCSectorInfo> _tx_sectors;
