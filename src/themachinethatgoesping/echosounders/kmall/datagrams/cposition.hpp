...
#include ".docstrings/cposition.doc.hpp"
 ...
 */
class CPosition : public KMALLSensorDatagram
{
  public:
    static constexpr auto DatagramIdentifier = t_KMALLDatagramIdentifier::C_POSITION;
...
#pragma pack(pop)
  ...
  public:
    ...
    // ----- operators -----
    bool operator==(const CPosition& other) const = default;

    //----- to/from stream functions -----
    static CPosition from_stream(std::istream& is, const KMALLDatagram& header);

    static CPosition from_stream(std::istream& is, o_KMALLDatagramIdentifier datagram_identifier);

    static CPosition from_stream(std::istream& is);
    ...
    __STREAM_DEFAULT_TOFROM_BINARY_FUNCTIONS_NOT_CONST__(CPosition)

  private:
    explicit CPosition(const KMALLDatagram& header)
