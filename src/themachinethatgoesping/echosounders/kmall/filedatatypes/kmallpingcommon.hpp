...
#include ".docstrings/kmallpingcommon.doc.hpp"
...
{
  protected:
    std::string class_name() const { return "KMALLPingCommon"; }
    // raw data
    using t_rawdata                       = KMALLPingFileData<t_ifstream>;
  ...
  public:
    ...
    KMALLPingCommon(std::shared_ptr<t_rawdata> file_data)
        : _file_data(std::move(file_data))
    {
    }
    virtual ~KMALLPingCommon() = default;
