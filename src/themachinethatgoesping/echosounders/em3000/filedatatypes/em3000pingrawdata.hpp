...
class EM3000PingRawData : public filedatainterfaces::EM3000DatagramInterface<t_ifstream>
{
  ...
  public:
    ...
    std::vector<float>        get_selected_beam_pointing_angles() const
    {
        ...
        for (const auto& bn : _selected_beam_numbers)
            selected_beam_pointing_angles.push_back(_beam_pointing_angles.at(bn));
    ...
    uint16_t get_number_of_selected_beams() const { return _selected_beam_numbers.size(); }
    ...
    std::vector<uint16_t>               _number_of_samples_per_beam;
    ...
    void load_datagrams(bool skip_data = true)
    {
        ...
        _number_of_samples_per_beam.clear();
        ...
        for (const auto& b : water_column_datagram.beams())
        {
            ...
            _number_of_samples_per_beam.push_back(b.get_number_of_samples());
    ...
    {
        _selected_beam_numbers.clear();
        for (size_t bn = 0; bn < _beam_pointing_angles.size(); ++bn)
        {
            if ((_beam_pointing_angles[bn] >= min_beam_angle) &&
                (_beam_pointing_angles[bn] <= max_beam_angle))
            {
    ...
    xt::xtensor<float, 2> read_selected_samples()
    {
        ...
        for (const auto bn : _selected_beam_numbers)
        {
            size_t number_of_samples_to_read = _selected_number_of_samples_per_beam.at(bn);
            size_t sample_offset             = _selected_first_sample_per_beam.at(bn) -
            ...
            xt::xtensor<int8_t, 1> beam_samples =
                datagrams::substructures::WaterColumnDatagramBeam::read_samples(
                    ...
                    number_of_samples_to_read,
                    _number_of_samples_per_beam[bn]);
  ...
  public:
    ...
    tools::classhelper::ObjectPrinter __printer__(unsigned int float_precision) const
    {
        ...
        printer.register_container("number_of_samples_per_bean", _number_of_samples_per_beam);
