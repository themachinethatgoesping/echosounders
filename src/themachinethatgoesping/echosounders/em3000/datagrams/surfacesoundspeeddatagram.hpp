...
class SurfaceSoundSpeedDatagram : public EM3000Datagram
{
  ...
  private:
    ...
    explicit SurfaceSoundSpeedDatagram(EM3000Datagram header)
  ...
  public:
    ...
    {
        _datagram_identifier = t_EM3000DatagramIdentifier::SurfaceSoundSpeedDatagram;
    }
    ...
    // ----- operators -----
    bool operator==(const SurfaceSoundSpeedDatagram& other) const
    {
    ...
    static SurfaceSoundSpeedDatagram from_stream(std::istream& is, EM3000Datagram header)
    {
        SurfaceSoundSpeedDatagram datagram(std::move(header));

        if (datagram._datagram_identifier != t_EM3000DatagramIdentifier::SurfaceSoundSpeedDatagram)
            throw std::runtime_error(fmt::format(
                "SurfaceSoundSpeedDatagram: datagram identifier is not 0x{:02x}, but 0x{:02x}",
                uint8_t(t_EM3000DatagramIdentifier::SurfaceSoundSpeedDatagram),
