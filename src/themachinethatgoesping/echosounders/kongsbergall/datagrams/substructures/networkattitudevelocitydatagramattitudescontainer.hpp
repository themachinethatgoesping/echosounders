...
#include ".docstrings/networkattitudevelocitydatagramattitudescontainer.doc.hpp"
...
class NetworkAttitudeVelocityDatagramAttitudesContainer
{
  public:
    NetworkAttitudeVelocityDatagramAttitudesContainer()  = default;
    ~NetworkAttitudeVelocityDatagramAttitudesContainer() = default;

    // ----- vector access -----
    const std::vector<NetworkAttitudeVelocityDatagramAttitude>& get_attitudes() const;
    std::vector<NetworkAttitudeVelocityDatagramAttitude>&       attitudes();
    void set_attitudes(const std::vector<NetworkAttitudeVelocityDatagramAttitude>& attitudes);

    // Vector-like compatibility helpers
    std::size_t size() const { return _attitudes.size(); }
    const NetworkAttitudeVelocityDatagramAttitude& operator[](std::size_t idx) const
    {
        return _attitudes[idx];
    }
    NetworkAttitudeVelocityDatagramAttitude& operator[](std::size_t idx) { return _attitudes[idx]; }
    ...
    xt::xtensor<uint16_t, 1> get_number_of_bytes_in_input_datagram_tensor(
        const std::vector<uint32_t>& indices = {}) const;
    ...
    // ----- operators -----
    bool operator==(const NetworkAttitudeVelocityDatagramAttitudesContainer& other) const = default;
  ...
  private:
    std::vector<NetworkAttitudeVelocityDatagramAttitude> _attitudes;
