...
constexpr float backscatter_scale     = 0.1;
constexpr float incidence_angle_scale = 0.1;
...
void XYZDatagramBeam::set_beam_incidence_angle_adjustment(float beam_incidence_angle_adjustment)
{
...
float XYZDatagramBeam::get_backscatter() const
{
    return static_cast<float>(_reflectivity) * backscatter_scale;
}

float XYZDatagramBeam::get_beam_incidence_angle_adjustment_in_degrees() const
{
    return static_cast<float>(_beam_incidence_angle_adjustment) * incidence_angle_scale;
}
...
// ---- processed -----
float XYZDatagramBeam::get_beam_incidence_angle_horizontal_plane_in_degrees() const 
{
    constexpr float to_degrees = 180.0f / static_cast<float>(M_PI);

    return to_degrees * std::atan(_depth / std::abs(_acrosstrack_distance)) +
...
tools::classhelper::ObjectPrinter XYZDatagramBeam::__printer__(unsigned int float_precision,
                                                               bool superscript_exponents) const
{
    ...
    printer.register_value(
        "beam_incidence_angle_adjustment", get_beam_incidence_angle_adjustment_in_degrees(), "°");
	...
	printer.register_value("beam_incidence_angle_horizontal_plane", get_beam_incidence_angle_horizontal_plane_in_degrees(), "°");
