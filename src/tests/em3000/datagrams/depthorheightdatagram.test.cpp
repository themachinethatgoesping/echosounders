...
TEST_CASE("DepthOrHeightDatagram should support common functions", TESTTAG)
{
    // initialize class structure
    auto dat = DepthOrHeightDatagram();
    ...
    REQUIRE(dat == DepthOrHeightDatagram(dat.from_stream(buffer)));
    ...
    REQUIRE(dat.get_height_in_meters() == Catch::Approx(1.23));
    REQUIRE(dat.get_height_type() == 100);

    // datagram type
    REQUIRE(dat.get_datagram_identifier() == t_EM3000DatagramIdentifier::DepthOrHeightDatagram);
