...
TEST_CASE("DatagramInfoDatashould support common functions", TESTTAG)
{
    ...
    auto obj  = SimradRawWaterColumnCalibration(sound_velocity,
                                                  effective_pulse_duration,
                                                  system_gain_offset,
                                                  tvg_absorption_db_m,
                                                  tvg_factor);
    auto obj2 = SimradRawWaterColumnCalibration();
    ...
    // test copy
    {
        ...
        INFO(SimradRawWaterColumnCalibration(obj).info_string());
        CHECK(obj == SimradRawWaterColumnCalibration(obj));
    }

    // test binary
    CHECK(obj == SimradRawWaterColumnCalibration(obj.from_binary(obj.to_binary())));

    // test stream
    std::stringstream buffer;
    obj.to_stream(buffer);
    CHECK(obj == SimradRawWaterColumnCalibration(obj.from_stream(buffer)));
