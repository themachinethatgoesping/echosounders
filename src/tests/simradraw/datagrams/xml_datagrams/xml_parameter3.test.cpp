...
using namespace themachinethatgoesping::echosounders::simradraw::datagrams::xml_datagrams;
...
TEST_CASE("ChannelConfiguration should support common functions", TESTTAG)
{
    ...
    REQUIRE(ChannelConfiguration() == ChannelConfiguration());

    // test copy
    // channel1.print(std::cerr);
    // XML_Configuration(channel1).print(std::cerr);
    REQUIRE(channel1 == ChannelConfiguration(channel1));
    REQUIRE(channel2 == ChannelConfiguration(channel2));

    // test binary
    REQUIRE(channel1 == ChannelConfiguration::from_binary(channel1.to_binary()));
    REQUIRE(channel2 == ChannelConfiguration::from_binary(channel2.to_binary()));

    // test stream
    std::stringstream buffer;
    channel1.to_stream(buffer);
    REQUIRE(channel1 == ChannelConfiguration::from_stream(buffer));

    // test print does not crash
    REQUIRE(channel1.info_string().size() != 0);
    ...
    REQUIRE(channel2.get_channel_id() == "WBT 400050-15 ES70-7C_ES");

    REQUIRE(channel1.get_sensor_offsets() == offsets_1);
    REQUIRE(channel2.get_sensor_offsets() == offsets_2);
}