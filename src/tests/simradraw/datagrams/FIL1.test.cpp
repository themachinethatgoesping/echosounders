...
TEST_CASE("FIL1 should support common functions", TESTTAG)
{
    ...
    auto coeff =
        xt::xtensor<std::complex<simradraw_float>, 1>({ { 1, 5 }, { 2, 6 }, { 3, 7 }, { 4, 8 } });
    ...
    REQUIRE(themachinethatgoesping::tools::helper::approx_container_complex(coeff2, coeff));

    auto xcoeff = dat.get_coefficients();
    REQUIRE(xcoeff.shape()[0] == 4);
    REQUIRE(xcoeff(0) == std::complex<simradraw_float>(1, 5));
    REQUIRE(xcoeff(1) == std::complex<simradraw_float>(2, 6));
    ...
    REQUIRE(dat.get_length() ==
            simradraw_long(12 + 136 + dat.get_no_of_coefficients() * sizeof(std::complex<simradraw_float>)));
}
