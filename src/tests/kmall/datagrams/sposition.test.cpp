...
TEST_CASE("SPosition should support common functions", TESTTAG)
{
    ...
    // test copy
    {
        ...
        INFO(fmt::format("copy constructor: {}", SPosition(dat).info_string()));
        REQUIRE(dat == SPosition(dat));
    }

    // test binary
    {
        auto dat2 = SPosition(dat.from_binary(dat.to_binary()));
        INFO(fmt::format("orig: {}", dat.info_string()));
    ...
    // datagram type
    REQUIRE(dat.get_datagram_identifier() == t_KMALLDatagramIdentifier::S_POSITION);
