...
TEST_CASE("SClock should support common functions", TESTTAG)
{
    ...
    // SClock variables
    dat.set_offset_sec(3.2);
    dat.set_clock_dev_pu_microsec(2);
    dat.set_clock_data_from_sensor(test_text); // this should modify all size related variables

    {
        INFO(fmt::format("orig: {}", dat.info_string()));
        CHECK(dat.binary_hash() == 14026969719085002882ULL);
    }
    ...
    {
        INFO(fmt::format("orig: {}", dat.info_string()));
        INFO(fmt::format("copy constructor: {}", SClock(dat).info_string()));
        REQUIRE(dat == SClock(dat));
    }

    // test binary
    {
        auto dat2 = SClock(dat.from_binary(dat.to_binary()));
        INFO(fmt::format("orig: {}", dat.info_string()));
    ...
    // test stream
    {
        ...
        auto dat2 = SClock(dat.from_stream(buffer));
        INFO(fmt::format("orig: {}", dat.info_string()));
    ...
    REQUIRE(dat.get_offset_sec() == Catch::Approx(3.2f));
    REQUIRE(dat.get_clock_dev_pu_microsec() == 2);

    // check pos data string
    REQUIRE(dat.get_clock_data_from_sensor().substr(0, test_text.size()) == test_text);

    // datagram type
    REQUIRE(dat.get_datagram_identifier() == t_KMALLDatagramIdentifier::S_CLOCK);
