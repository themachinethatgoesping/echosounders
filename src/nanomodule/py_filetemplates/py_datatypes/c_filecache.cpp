...
void init_c_filecache(nanobind::module_& m)
{
    ...
    auto cls =
        nb::class_<FileCache>(
            ...
            .def(nb::init<const std::string&,
                 ...
                 nb::arg("file_name"),
                 nb::arg("file_size"),
                 nb::arg("cache_keys") = std::vector<std::string>(),
                 DOC_FileCache(FileCache))
            .def("__eq__", &FileCache::operator==, DOC_FileCache(operator_eq), nb::arg("other"))
            .def("update_file",
                 &FileCache::update_file,
                 DOC_FileCache(update_file),
                 nb::arg("index_path"),
                 nb::arg("emulate_only") = false)
            .def_static(
                "from_file", &FileCache::from_file, DOC_FileCache(from_file), nb::arg("index_path"))
            ...
            .def(
                "has_cache", &FileCache::has_cache, DOC_FileCache(has_cache), nb::arg("cache_name"))

            // cache handling
            //   .def(
            //       "add_cache", &FileCache::template add_cache<std::string>,
            //       DOC_FileCache(add_cache), nb::arg("cache_name"))
            //   .def(
            //       "get_cache", &FileCache::template get_cache<std::string>,
            //       DOC_FileCache(get_cache), nb::arg("cache_name"))
