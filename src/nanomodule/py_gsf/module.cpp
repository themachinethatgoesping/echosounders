...
void init_c_gsffilehandler(nanobind::module_& m); // c_gsffilehandler.cpp
...
void init_m_gsf(nanobind::module_& m)
{
    ...
    // module description
    auto subm = m.def_submodule("gsf", "Classes related to GSF data files");

    nb::enum_<t_GSFDatagramIdentifier>(
            subm,
            "t_GSFDatagramIdentifier",
            DOC(themachinethatgoesping, echosounders, gsf, t_GSFDatagramIdentifier))
        .value("HEADER", t_GSFDatagramIdentifier::HEADER, DOC_t_GSFDatagramIdentifier(HEADER))
    ...
    themachinethatgoesping::tools::nanobind_helper::make_option_class<o_GSFDatagramIdentifier>(
        subm,
        "o_GSFDatagramIdentifier");

    subm.def("datagram_type_to_string",
             nb::overload_cast<t_GSFDatagramIdentifier>(&datagram_type_to_string),
