...
void init_c_i_raw3data(nanobind::module_& m);             // c_i_raw3data.cpp
void init_c_raw3datacomplexfloat32(nanobind::module_& m); // c_i_raw3data_power.cpp
void init_c_raw3datapowerandangle(nanobind::module_& m);  // c_i_raw3data_power_and_angle.cpp
void init_c_raw3datapower(nanobind::module_& m);          // c_i_raw3data_power.cpp
void init_c_raw3dataangle(nanobind::module_& m);          // c_i_raw3data_angle.cpp
void init_c_raw3dataskipped(nanobind::module_& m);        // c_i_raw3data_skipped.cpp

// -- create submodule --
void init_m_raw3datatypes(nanobind::module_& m)
{
    // module description
    auto subm = m.def_submodule(
        "raw3datatypes", "SimradRaw EK80 sample datagram data structures used in RAW3 datagrams).");

    using themachinethatgoesping::echosounders::simradraw::datagrams::raw3datatypes::t_RAW3DataType;

    // enums
    auto pyenum_raw3_datatype = nb::enum_<t_RAW3DataType>(subm,
                                  "t_RAW3DataType",
                                  DOC(themachinethatgoesping,
                                      echosounders,
                                      simradraw,
                     ...
                     .value("PowerAndAngle",
                         ...
                         DOC_T_RAW3_DATATYPE(PowerAndAngle))
