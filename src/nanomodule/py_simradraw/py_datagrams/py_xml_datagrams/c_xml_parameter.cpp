// SPDX-FileCopyrightText: 2022 - 2025 Peter Urban, Ghent University
// SPDX-FileCopyrightText: 2022 Peter Urban, GEOMAR Helmholtz Centre for Ocean Research Kiel
//
// SPDX-License-Identifier: MPL-2.0

#include <nanobind/nanobind.h>
#include <nanobind/stl/vector.h>

#include <themachinethatgoesping/tools_nanobind/classhelper.hpp>

#include "module.hpp"
#include <themachinethatgoesping/echosounders/simradraw/datagrams/xml_datagrams/xml_parameter.hpp>

namespace themachinethatgoesping {
namespace echosounders {
namespace pymodule {
namespace py_simradraw {
namespace py_datagrams {
namespace py_datagrams_xml {

namespace nb = nanobind;
using namespace themachinethatgoesping::echosounders::simradraw;
using datagrams::xml_datagrams::XML_Parameter;

void init_c_xml_parameter(nanobind::module_& m)
{
    nb::class_<XML_Parameter>(
        m,
        "XML_Parameter",
        DOC(themachinethatgoesping, echosounders, simradraw, datagrams, xml_datagrams, XML_Parameter))
        .def(nb::init<>(),
             DOC(themachinethatgoesping,
                 echosounders,
                 simradraw,
                 datagrams,
                 xml_datagrams,
                 XML_Parameter,
                 XML_Parameter))

        // ----- attributes -----
        .def_rw("Channels",
                       &XML_Parameter::Channels,
                       DOC(themachinethatgoesping,
                           echosounders,
                           simradraw,
                           datagrams,
                           xml_datagrams,
                           XML_Parameter,
                           Channels))

        // ----- methods -----
        .def("parsed_completely",
             &XML_Parameter::parsed_completely,
             DOC(themachinethatgoesping,
                 echosounders,
                 simradraw,
                 datagrams,
                 xml_datagrams,
                 XML_Parameter,
                 parsed_completely))

        // ----- pybind macros -----
        // default copy functions
        __PYCLASS_DEFAULT_COPY__(XML_Parameter)
        // default binary functions
        __PYCLASS_DEFAULT_BINARY__(XML_Parameter)
        // default printing functions
        __PYCLASS_DEFAULT_PRINTING__(XML_Parameter)
        // end LinearInterpolator
        ;
}
}
}
}
}
}
}