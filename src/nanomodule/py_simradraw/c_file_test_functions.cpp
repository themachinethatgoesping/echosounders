...
using themachinethatgoesping::tools::nanobind_helper::ostream_redirect;
...
void init_c_file_test_functions(nanobind::module_& m)
{
    m.def("test_speed_raw",
        &test_speed_content<datagrams::SimradRawUnknown>,
        nb::call_guard<ostream_redirect>());
    m.def("test_speed_type", &test_speed_type, nb::call_guard<ostream_redirect>());
    ...
    m.def("test_speed_decode_xml",
        ...
        nb::call_guard<ostream_redirect>(),
          nb::arg("mapped_file_stream"),
          nb::arg("level") = 10);
    m.def("test_speed_raw_all", &test_speed_all, nb::call_guard<ostream_redirect>());
    m.def("test_speed_header", &test_speed_header, nb::call_guard<ostream_redirect>());
}
