...
void init_c_absorptionloss(nb::module_& m)
{
    nb::class_<AbsorptionLoss, S7KDatagram>(m, "AbsorptionLoss", DOC(themachinethatgoesping, echosounders, s7k, datagrams, AbsorptionLoss))
        ...
        .def("get_checksum", &AbsorptionLoss::get_checksum, DOC_C(AbsorptionLoss, Content, checksum))
        .def("set_checksum", &AbsorptionLoss::set_checksum, DOC_C(AbsorptionLoss, Content, checksum), nb::arg("val"))
