...
void init_c_spreadingloss(nb::module_& m)
{
    nb::class_<SpreadingLoss, S7KDatagram>(m, "SpreadingLoss", DOC(themachinethatgoesping, echosounders, s7k, datagrams, SpreadingLoss))
        ...
        .def("get_checksum", &SpreadingLoss::get_checksum, DOC_C(SpreadingLoss, Content, checksum))
        .def("set_checksum", &SpreadingLoss::set_checksum, DOC_C(SpreadingLoss, Content, checksum), nb::arg("val"))
