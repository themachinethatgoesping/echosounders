...
void init_c_referencepoint(nb::module_& m)
{
    nb::class_<ReferencePoint, S7KDatagram>(m, "ReferencePoint", DOC(themachinethatgoesping, echosounders, s7k, datagrams, ReferencePoint))
        ...
        .def("get_checksum", &ReferencePoint::get_checksum, DOC_C(ReferencePoint, Content, checksum))
        .def("set_checksum", &ReferencePoint::set_checksum, DOC_C(ReferencePoint, Content, checksum), nb::arg("val"))
