...
#define LOCAL_DOC_PREFIX                                                                            \
    themachinethatgoesping, echosounders, s7k, filedatainterfaces, S7KConfigurationDataInterface

template<typename T_FileStream>
void py_create_class_S7KConfigurationDataInterface(nanobind::module_& m,
                                                   const std::string& CLASS_NAME)
{
    using py_filetemplates::py_datainterfaces::py_i_configurationdatainterface::
        ConfigurationDataInterface_add_interface;

    using T_BaseClass = filedatainterfaces::S7KConfigurationDataInterface<T_FileStream>;
...
{
    static const std::string name        = "S7KConfigurationDataInterface";
    static const std::string name_stream = name + "_stream";
    static const std::string name_mapped = name + "";

    py_create_class_S7KConfigurationDataInterface<std::ifstream>(m, name_stream);
    py_create_class_S7KConfigurationDataInterface<datastreams::MappedFileStream>(m, name_mapped);
}
