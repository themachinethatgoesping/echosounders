...
void init_c_puidoutput(nanobind::module_& m)
{
     nb::class_<PUIDOutput, datagrams::KongsbergAllDatagram>(
        m, "PUIDOutput", DOC(themachinethatgoesping, echosounders, kongsbergall, datagrams, PUIDOutput))
     .def(nb::init<>(), DOC_PUIDOutput(PUIDOutput))
        ...
        // ----- operators -----
     .def("__eq__", &PUIDOutput::operator==, DOC_PUIDOutput(operator_eq), nb::arg("other"))
        // ----- pybind macros -----
