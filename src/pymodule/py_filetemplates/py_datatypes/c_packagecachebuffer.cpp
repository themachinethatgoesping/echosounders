...
#define DOC_PackageCacheBuffer(ARG)                                                                \
    DOC(themachinethatgoesping, echosounders, filetemplates, datatypes, PackageCacheBuffer, ARG)
...
void init_packagecachebuffer(pybind11::module& m, const std::string& typestr = "")
{
    ...
    auto cls =
        py::class_<t_PackageCacheBuffer, std::shared_ptr<t_PackageCacheBuffer>>(
            m,
            fmt::format("PackageCacheBuffer<{}>", typestr).c_str(),
            DOC(themachinethatgoesping, echosounders, filetemplates, datatypes, PackageCacheBuffer))
