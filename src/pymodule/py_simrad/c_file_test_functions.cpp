...
void test_speed_all(const FileSimradRaw<datastreams::MappedFileStream>& ifi)
{
    // get current time
    auto                     start = std::chrono::high_resolution_clock::now();
    datagrams::SimradUnknown a;

    auto it  = ifi.datagram_interface().datagrams<datagrams::SimradUnknown>();
...
void test_speed_content(const FileSimradRaw<datastreams::MappedFileStream>& ifi,
...
void test_speed_decode_nmea(const FileSimradRaw<datastreams::MappedFileStream>& ifi)
{
    // get current time
    auto start = std::chrono::high_resolution_clock::now();

    auto it = ifi.datagram_interface().datagrams<datagrams::NME0>(t_SimradDatagramIdentifier::NME0);
    auto prg = themachinethatgoesping::tools::progressbars::ProgressIndicator();
    prg.init(0., double(it.size()), "test reading");
...
void test_speed_decode_xml(const FileSimradRaw<datastreams::MappedFileStream>& ifi, int level = 10)
{
    // get current time
    auto start = std::chrono::high_resolution_clock::now();

    auto it = ifi.datagram_interface().datagrams<datagrams::XML0>(t_SimradDatagramIdentifier::XML0);
    auto prg = themachinethatgoesping::tools::progressbars::ProgressIndicator();
    prg.init(0., double(it.size()), "test reading");
...
void test_speed_type(const FileSimradRaw<datastreams::MappedFileStream>& ifi,
                     t_SimradDatagramIdentifier                          type)
{
    switch (type)
    {
...
void test_speed_header(const FileSimradRaw<datastreams::MappedFileStream>& ifi,
                       t_SimradDatagramIdentifier                          type)
{
    // get current time
    auto                     start = std::chrono::high_resolution_clock::now();
    ...
    auto it  = ifi.datagram_interface().datagrams<datagrams::SimradDatagram>(type);
    auto prg = themachinethatgoesping::tools::progressbars::ProgressIndicator();
    prg.init(0., double(it.size()), "test reading");
